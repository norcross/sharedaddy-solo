...
class Share_Email extends Sharing_Source {
	...
	// log the email
	public function log_process( $data ) {

		$post_id	= $data['id'];

		$current	= get_post_meta( $post_id, '_shared_emails', true );
		$updates	= array();
		...
		for ( $i = 0; $i < $count; $i++ ) {
			...
			$updates[$i]['id']				= $data['id'];
			$updates[$i]['time']			= $data['time'];
			$updates[$i]['source-name']		= $data['source-name'];
			$updates[$i]['source-email']	= $data['source-email'];
			$updates[$i]['target-email']	= $data['target-email'];
			$updates[$i]['message']			= $data['message'];

		}

		if ( !empty($current) )
			$updates = array_merge($current, $updates);

		if ( !empty( $updates ) && $updates != $current )
			update_post_meta($post_id, '_shared_emails', $updates);

		elseif ( empty($updates) && $current )
			delete_post_meta($post_id, '_shared_emails', $updates);
	...
	public function process_request( $post, array $post_data ) {
		...
		if ( isset( $post_data['target_message'] ) )
			$target_message = $post_data['target_message'];
...
?>
	<div id="sharing_email" style="display: none;">
		<form action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>" method="post">
			...
			<div class="sd-field-block">
				<label for="target_message"><?php _e( 'Message', 'jetpack' ) ?></label>
				<textarea name="target_message" id="target_message" cols="40" rows="3"></textarea>
			</div>
