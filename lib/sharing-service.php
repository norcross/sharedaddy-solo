...
function sharing_add_header() {
	...
	if ( count( $enabled['all'] ) > 0 )
		wp_enqueue_style( 'sharedaddy', plugin_dir_url( __FILE__ ) .'css/sharing.css', array(), null );
}
...
function sharing_display( $text = '' ) {
	...
	if ( $show ) {
		...
		if ( count( $enabled['all'] ) > 0 ) {
			...
			// Register our JS
			wp_register_script( 'sharing-js', plugin_dir_url( __FILE__ ).'js/sharing.js', array( 'jquery' ), '20121205' );
